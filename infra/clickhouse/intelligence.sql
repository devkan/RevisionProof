...
-- Opt-in uploaded scene indexing. Only derived summaries/embeddings are stored, and expire in 7 days.
CREATE TABLE IF NOT EXISTS revisionproof.smart_scene_segments
(
    workspace_id String,
    search_id FixedString(26),
    ...
    INDEX smart_scene_hnsw embedding TYPE vector_similarity('hnsw', 'cosineDistance', 768) GRANULARITY 100000000,
    CONSTRAINT smart_scene_embedding_dimensions CHECK length(embedding) = 768,
    CONSTRAINT smart_scene_time CHECK start_seconds >= 0 AND end_seconds > start_seconds
)
ENGINE = MergeTree
ORDER BY (workspace_id, search_id, asset_id, start_seconds, segment_id)
TTL expires_at DELETE;

GRANT SELECT ON revisionproof.smart_scene_segments TO revisionproof_view_definer;

CREATE VIEW IF NOT EXISTS revisionproof.smart_scene_search
DEFINER = revisionproof_view_definer_user SQL SECURITY DEFINER AS
SELECT workspace_id, search_id, segment_id, asset_id, start_seconds, end_seconds,
    transcript, visual_summary, embedding, expires_at
FROM revisionproof.smart_scene_segments;

GRANT INSERT ON revisionproof.smart_scene_segments TO revisionproof_writer;
GRANT SELECT ON revisionproof.smart_scene_search TO revisionproof_mcp_reader;

CREATE TABLE IF NOT EXISTS revisionproof.approved_edit_recipes
(
    workspace_id String,
    memory_id FixedString(64),
    run_id FixedString(26),
    ...
    proof_json String,
    approved_at DateTime64(3, 'UTC'),
    INDEX approved_recipe_hnsw embedding TYPE vector_similarity('hnsw', 'L2Distance', 768) GRANULARITY 100000000,
    CONSTRAINT recipe_embedding_dimensions CHECK length(embedding) = 768,
    CONSTRAINT recipe_candidate CHECK candidate_id IN ('A', 'B'),
    CONSTRAINT recipe_duration CHECK duration_seconds >= 0.1 AND duration_seconds <= 60.05
)
ENGINE = MergeTree
ORDER BY (workspace_id, embedding_model, memory_id);

GRANT SELECT ON revisionproof.approved_edit_recipes TO revisionproof_view_definer;

CREATE VIEW IF NOT EXISTS revisionproof.approved_edit_recipe_memory
DEFINER = revisionproof_view_definer_user SQL SECURITY DEFINER AS
SELECT workspace_id, memory_id, run_id, spec_hash, intent, target_phrase, candidate_id,
    duration_seconds, operations_json, embedding_model, embedding, embedding_qbit, approved_at
FROM revisionproof.approved_edit_recipes;

CREATE VIEW IF NOT EXISTS revisionproof.approved_edit_recipe_neighbors
DEFINER = revisionproof_view_definer_user SQL SECURITY DEFINER AS
SELECT memory_id, intent, target_phrase, candidate_id, duration_seconds, operations_json,
    approved_at, spec_hash, L2Distance(embedding, {reference_vector:Array(Float32)}) AS distance
FROM revisionproof.approved_edit_recipes
WHERE workspace_id = {workspace:String} AND embedding_model = {model:String}
ORDER BY distance ASC LIMIT 15;

GRANT INSERT ON revisionproof.approved_edit_recipes TO revisionproof_writer;
GRANT SELECT ON revisionproof.approved_edit_recipe_memory TO revisionproof_mcp_reader;
GRANT SELECT ON revisionproof.approved_edit_recipe_neighbors TO revisionproof_mcp_reader;
