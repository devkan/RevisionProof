...
import { checkedVideoUrl, countedLabel, planAction, previewGuidance, reviewedPlan, uploadPreparationRange, type ExternalVideo } from './studioWorkflow'
import type {
  DemoAsset,
  EditOperation,
  PatchCandidate,
...
const PROCESSING_COPY: Record<ProcessingPhase, { title: string; detail: string }> = {
  ...
  previews: { title: 'Rendering your edits…', detail: 'Applying only the edits you selected. You will choose the version.' },
    ...
    return (
      <aside ref={leftColumn} className="studio-left studio-scroll-column">
        ...
        <ol className="studio-checklist">
          ...
          <li className={displayOperations.length ? 'is-done' : ''}>{displayOperations.length ? <Check size={14} /> : <span>2</span>}<span><strong>Edits added</strong><small>{countedLabel(displayOperations.length, 'item')}</small></span></li>
          <li className={processed ? 'is-done' : ''}>{processed ? <Check size={14} /> : <span>3</span>}<span><strong>Previews ready</strong><small>{processed ? countedLabel(run?.candidates.length ?? 0, 'version') : 'Not started'}</small></span></li>
  ...
  const primaryLabel = existingPlanAction?.label ?? (step === 1 ? 'Continue to edits'
    : step === 2 ? `Check plan (${operations.length})`
      : step === 3 ? run?.edit_plan ? 'Create previews' : 'Check plan'
        : step === 4 ? run?.spec ? 'Continue to checks' : previewGuidance(run?.candidates.length ?? 0).action
