...
export function EditMemory({ run, disabled, runtime, onApply }: { run: RunSnapshot; disabled: boolean; runtime?: RuntimeStatus; onApply?: (plan: EditPlan, label: string) => void }) {
  ...
  return <details className="edit-memory memory-compact" aria-busy={loading} onToggle={(event) => { if (event.currentTarget.open && !result && !loading) void search() }}>
    ...
    {!loading && result?.matches.map((match) => <article className="memory-match" key={match.memory_id}><div><span className="memory-verified"><CheckCircle2 size={16} />Verified + human approved</span><h4>{match.intent}</h4><p>“{match.target_phrase}” · {match.edit_plan ? `${match.edit_plan.operations.length} edits` : `${match.duration_seconds}s center punch-in`}</p>{match.edit_plan && onApply && <button type="button" className="secondary-button" disabled={disabled} onClick={() => onApply(match.edit_plan!, match.target_phrase)}><RefreshCw size={16} />Use as editable draft</button>}</div><div className="memory-recommendation"><strong>{match.edit_plan ? 'Edit recipe' : `Option ${match.candidate_id}`}</strong><span>{match.edit_plan ? `${match.duration_seconds.toFixed(1)}s output` : `${match.scale?.toFixed(2)}× crop`}</span><small>Similarity {match.similarity.toFixed(3)}</small></div></article>)}
