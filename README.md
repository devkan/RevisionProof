...
The billed GCP demo is available at [RevisionProof LIVE](https://revisionproof-staging-sdixpvvwoq-uc.a.run.app/?release=1b2c0ae). Revision `revisionproof-staging-00021-4fx` serves source `1b2c0ae`, including the guided basic editor and verified Vertex draft fixes. Fresh synthetic LIVE run `01M1KHABHY8YWTZ7RQVPK3MF9Q` used a real Korean Gemini draft, then combined a Korean subtitle, center zoom, English text, a timed cut and an explicitly selected silence cut. The complete Option B video shortened 10 seconds to 7.766667 seconds and reached `READY` with three PASS checks and an official MCP Change Map of 8 windows / 16 samples, with zero review flags. The downloaded MP4 exactly matched the approved preview. Korean/English frames, playback and the mapped comparison passed inspection. Final delivery approval remains false. The owner's actual KANAPP video passed local testing; LIVE QA used synthetic media. See the [current guide and release evidence](docs/basic-editing-guide-2026-09-03.md) for source/build/image IDs, tests and ready-to-paste requests. Previously opened tabs may need a hard refresh. The [preceding audio/upload fix](docs/kanapp-demo-fix-2026-09-03.md) and historical delivery-approved run `01M0Z3338TYVHWHK4FZRGADTKZ` remain separate evidence.
